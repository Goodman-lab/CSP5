...
class MolAPreprocessor(MolPreprocessor):
    ...
    def construct_feature_matrices(self, entry):
        ...
        connectivity = np.zeros((n_bond, 2), dtype='int')
        pos_matrix = np.zeros((n_atom,3), dtype='float')
        ...
        for i, atom in enumerate(mol.GetAtoms()):
            ...
            pos_matrix[i] = [positions.x, positions.y, positions.z]
        ...
        for n, atom in enumerate(atoms):
            ...
            for neighbor in neighbor_inds:
                ...
                try:
                    ...
                    # Fallback: if bond featurization fails for any reason, treat
                    # it as a non-bond edge rather than printing debug noise.
                    bond_feature_matrix[bond_index] = 0
        ...
        return {
            ...
            'n_bond': n_bond,
            'n_pro': n_pro,
            'node_attributes': atom_feature_matrix,
            'bond': bond_feature_matrix,
            'distance': bond_distance_matrix,
            'edge_indices': connectivity,
            'atom_index': atom_index_matrix,
            'node_coordinates':pos_matrix
        }
